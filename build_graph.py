...
def add_feature_from_pbtxt(g, pbtxt_data,model_name):
    if model_name == "cvae" or model_name == "gan" or model_name == "dcgan" or model_name == "conditionalgan" or model_name == "anomalydetection":
        tensor_shape_in_pbtxt = parsing.pbtxt_extract_tensor_shape_gan(pbtxt_data)
    else:
...
def add_feature_from_csv(g, node_type_file, pbtxt_data,model_name):
...
def build_nx_graph(pbtxt_path, node_type_file, dnn_model):
    ...
    #standard file name
    #model_name = dnn_model.split('_')[1]
    model_name = dnn_model
    if model_name == "cvae" or model_name == "gan" or model_name == "dcgan" or model_name == "conditionalgan" or model_name == "anomalydetection":
        op_node_name = parsing.pbtxt_extract_op_node_name_gan(pbtxt_data)
        op_node_edge = parsing.pbtxt_extract_op_node_edge_gan(pbtxt_data)
    else:
    ...
    add_feature_from_pbtxt(g, pbtxt_data,model_name)
    #add_feature_from_csv(g, node_type_file, pbtxt_data,model_name)
    #nodetype file excluded
    return g

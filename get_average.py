    ...
    for gpu in constants.gpu_model:
        for _dset in constants.hp_dataset:
            for model in tqdm(constants.hp_model_node_freq,desc='models'):
                ...
                model_average_exec_time = average_exec_time/model_iterations
                logfile.writelines("{0}_{1}_{2}; Average GPU Util%:{3} ,Average Mem Util%:{4} , Average Memory:{5}, Average CPU Util%:{6}, Average Execution Time:{7}\n"
                .format(gpu,model,_dset,model_average_util, model_average_mem_util,model_average_mem,model_average_cpu_util,model_average_exec_time))
    print(perf_list)
